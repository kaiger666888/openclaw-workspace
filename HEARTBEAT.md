...
## ⚡ 即时汇报规则（2026-05-21 确立）

**每完成一个动作，立刻发消息汇报。不等工具返回，不等用户催。**

1. 开始任务 → 发一条"正在做X"
2. 工具返回结果 → 发一条"X完成了，结果是Y"
3. 遇到问题 → 发一条"X遇到问题：原因，正在尝试方案Z"
4. 等待中（API/生成）→ 发一条"正在等待X完成，预计Y秒"

**绝对不允许：默默执行完后等用户问"怎么样了"才回复。**

这条规则适用于所有长任务：图像生成、视频生成、脚本执行、API调用等。

## 📝 夜间回顾总结 (2026-02-25)

### 🌙 夜间回顾任务执行完成
**任务ID**: 6d8395cb-66ad-44e3-8bc6-210177c8af9f  
**执行时间**: 2026-02-25 凌晨 01:00 (Asia/Shanghai)  
**状态**: ✅ 成功完成

### ✅ 更新内容
- **知识管理系统架构验证**: 首次执行夜间回顾任务，成功验证 MEMORY.md + memory/YYYY-MM-DD.md + HEARTBEAT.md + 夜间回顾任务全流程
- **项目状态更新**: UML Vision Agent 提升至 P1 优先级，MVP 状态确认完成
- **核心决策记录**: 新增4个关键决策（知识管理系统架构、主动知识捕获策略、UML Agent MVP 完成、知识可视化系统扩展）
- **优先级优化**: 调整项目排序，突出活跃项目状态

### 📊 今日统计
- 对话轮次：4个
- 新增决策：4个
- 项目进展：2个提升（UML Agent 提升至P1，知识管理系统验证完成）
- 系统健康：✅ 夜间回顾任务架构验证通过

### 🎯 系统架构成果
- **渐进式总结验证**: Layer 1原始笔记 → Layer 2重要内容 → Layer 3优先级 → Layer 4核心洞察
- **PARA结构运行**: Projects/Areas/Resources/Archives 分类管理正常运行
- **自动化知识捕获**: HEARTBEAT.md 实时记录策略确立，支持即时知识积累
...
### 专家管理命令
- 如果收到 "/expert status {name}" → 运行 `/opt/openclaw/scripts/expert_manager.sh status {name}` 并返回结果
- 如果收到 "/expert approve {name} {id}" → 运行 `/opt/openclaw/scripts/expert_manager.sh approve {name} {proposal_path}`
- 如果收到 "/expert reject {name} {id} --reason {reason}" → 运行 `/opt/openclaw/scripts/expert_manager.sh reject {name} {proposal_path} {reason}`
- 如果收到 "/expert rollback {name} --to {version} --reason {reason}" → 运行 `/opt/openclaw/scripts/expert_manager.sh rollback {name} {version} {reason}`

### 专家晋升扫描
- 如果收到 "promotion-scan" → 运行 `/opt/openclaw/scripts/run_promotion_scan.sh`，汇报扫描结果（哪些专家触发了学习提案）

### 进化周期检查
- 如果收到 "evolution-cycle" → 运行 `/opt/openclaw/scripts/evolution_cycle.sh`，汇报结果（已完成项目数、待审核提案、建议方向）
