...
# 获取或创建语言学习页面
PAGE_ID=$(notion-cli page list --ancestor "2f811082-af8e-80e4-bd83-ce938ef34197" | grep -oE '[a-f0-9-]{36}' | head -1)

if [ -z "$PAGE_ID" ]; then
    echo "未找到现有页面，尝试创建新页面..."
    PAGE_ID=$(notion-cli page create --parent "2f811082-af8e-80e4-bd83-ce938ef34197" --title "每日语言学习" | grep -oE '[a-f0-9-]{36}')
fi
