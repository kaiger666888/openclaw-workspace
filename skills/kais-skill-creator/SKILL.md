...
**设计原则：**
- **尊重区**：设置 Human-in-the-loop 卡点（需求确认、发布确认）
- **增强区**：输出结构化数据（评分报告、优化建议）供人快速决策
- **自动化区**：Agent 直接执行（脚手架生成、评分、迭代优化）
- **半自动区**：AI 执行 → 人快速二选一 → 继续执行

### 各步骤自动化级别

| 步骤 | 区域 | 人工参与 | 说明 |
|------|------|---------|------|
| 首次向导 | 半自动 | 初始配置 | AI 提问 → 人回答 → AI 写入配置 |
| 需求理解 | 尊重 | 确认摘要 | AI 分析 → 人生成确认/修改 |
| 脚手架生成 | 自动化 | 无 | AI 全自动执行 |
| SKILL.md 生成 | 增强 | 审核内容 | AI 生成 → 人可审核 |
| 质量评估 | 自动化 | 无 | score.py 全自动 |
| 全覆盖测试 | 自动化 | 无 | validate.sh 全自动 |
| 迭代优化 | 自动化 | 仅卡住时 | AI 自动循环，仅 <60 分时暂停问人 |
| GitHub 推送 | 半自动 | 确认推送 | AI 准备 → 人确认 → AI 执行 |
| 交付确认 | 尊重 | 最终决策 | 人决定满意/调整/重来 |
| ClawHub 发布 | 尊重 | 确认发布 | AI 准备 checklist → 人确认 |
